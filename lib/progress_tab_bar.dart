...
class _ProgressTabBarState extends State<ProgressTabBar> {
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
        ...
        child: SizedBox(
            ...
            //TODO:高度设置
            height: widget.height + widget.outlineWidth + 1 - 10,
            child: Padding(
  ...
  List<ProgressTabButton> _buttonBuilder(
    ...
    for (int i = 0; i < tabs.length; i++) {
      ...
      tabButtons.add(ProgressTabButton(
        // width: width,
        //定义长度
        width: tabs[i].width ?? width,
        spacing: tabs[i].spacing ?? widget.spacing,
        height: height,
  ...
  Widget _stackBuilder(
      List<ProgressTabButton> tabButtons, double buttonSpacing) {
    List<Container> positionedTabs = new List.empty(growable: true);
    double xPos = 0;
    for (int i = 0; i < tabButtons.length; i++) {
      positionedTabs.add(Container(
        child: tabButtons[i],
        margin: EdgeInsets.only(right: buttonSpacing),
      ));
      // xPos +=
      //     // (widget.tabWidth - widget.tabWidth * 0.3 / 2) +
      //     // buttonSpacing +
      //     // widget.outlineWidth / 2;
      //     // buttonSpacing + tabButtons[i].width / 1.5;
      //     buttonSpacing + tabButtons[i].width +tabButtons[i].spacing;
    }
    return Row(
  ...
  double _calculateScrollOffset(ProgressTabBar widget) {
    return (widget.tabWidth -
                widget.tabWidth * 0.9 / 1.1 // Tab width
                +
                widget.spacing) // Tab Spacing
            *
            (widget.children.length - 2 < widget.selectedTab!
